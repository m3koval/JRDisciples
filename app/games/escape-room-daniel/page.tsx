...
export default function EscapeRoomDanielPage() {
  ...
  // Room background art (generated by APEX, see IMAGE_TASKS.md).
  // If a file is missing the layer is simply transparent — the gradient shows through.
  const ROOM_BG_NAMES = ['escape-room-daniel-decree', 'escape-room-daniel-window', 'escape-room-daniel-den', 'escape-room-daniel-victory']
  const roomBg = phase === 'victory'
    ? ROOM_BG_NAMES[3]
    : (['room1','room2','room3','room4'] as Phase[]).includes(phase) ? ROOM_BG_NAMES[roomIdx] : null

  return (
    <main style={{ minHeight: '100dvh', background: 'linear-gradient(180deg,#0c0a1e,#1a1040 50%,#2c1810)', userSelect: 'none', WebkitUserSelect: 'none', position: 'relative' }}>
      {roomBg && (
        <div aria-hidden="true" style={{ position: 'absolute', inset: 0, backgroundImage: `url(/images/jr/games/escape-room/${roomBg}.png)`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.22, pointerEvents: 'none' }} />
      )}
      <section style={{ maxWidth: 620, margin: '0 auto', padding: '28px 18px 72px', color: '#fff', position: 'relative', zIndex: 1 }}>
